...
- **Server** — `/api/catalyst/{ticker}` is **Solana-only, exact-svm**, priced at
  **1000 USDC base units = 0.001 USDC** (`maxAmountRequired="1000"`). It settles
...
- Per weekly run: `197 × 1000 units = 197,000 units = 0.197 USDC` (~$0.20) +
  negligible SOL gas. Default cap `WEEKLY_SPEND_CAP_UNITS=500000` (0.5 USDC/run).
...
| `WEEKLY_SPEND_CAP_UNITS` | var | hard cap in base units (default 500000) |
...
1. **Dry-run** (no funds): Actions → *x402 weekly buyer* → `mode: dry-run`. Confirms
   every endpoint returns a 402 whose challenge is Solana + `maxAmountRequired="1000"`.
...
```json
{ "summary": { "ok": 197, "spent_units": 197000, "spent_usdc": 0.197,
  "txs": [ { "ticker": "7212", "tx": "…", "solscan": "https://solscan.io/tx/…" } ] } }
```

and the workflow commits it, so the on-chain record of "paid ~197 per-call every week" is in git and clickable on Solscan.

## EDINET endpoint (same rail, separate sweep)

`/api/edinet/{code}` is a second paid endpoint on the **same** Solana exact-svm
rail (`withSolanaUsdcMicroPaywall`, 1000 units, network/mint/==units safety valve).
